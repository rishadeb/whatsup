...
yo = WhatsUp('./field_system_calsource')
options = yo.source_names_list
sourcename = ""
...
def update_graph(*args):
    ...
    global result
    global Fact
    hours = num_of_hours.get() if ((num_of_hours.get() > 0) and (num_of_hours.get() < 60)) else 10
    step_size = time_step_size.get() if (time_step_size.get() > 0) else 10
    source_name = source_clicked.get()

    result = yo.check_trajectory(hours*60*60, step_size*60, source_name) 
    ra.set(yo.get_ra_dec(source_clicked.get())[0])
    dec.set(yo.get_ra_dec(source_clicked.get())[1])
    ...
    if (plot_selected.get() == 'All'):
        ...
        for i in yo.source_names_list:
            answer = yo.check_trajectory(num_of_hours.get()*60*60, time_step_size.get()*60, i)
    ...
    for x in range(0,len(result[1])):
        Fact += 'azimuth = '+ str(result[1][x]) +' \t elevation = '+ str(result[2][x])+ " \t "+ katpoint.Timestamp(result[0][x]).to_string() + "\n"
    az_el_textbox.insert(tkinter.END, Fact)


result = yo.check_trajectory(num_of_hours.get()*60*60,1800,source_clicked.get())
Fact = ""
